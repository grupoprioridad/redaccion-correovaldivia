...
// Historias asignadas cuya fecha de entrega es mañana
$stmt = $db->query("
...
$contador = 0;
while ($h = $stmt->fetch()) {
    $subject = "🔔 ¡Entrega mañana! «{$h['titulo']}» vence";
    $msg = "
    <div style='font-family:sans-serif;max-width:600px;margin:0 auto'>
        <h2 style='color:#5e6ad2'>🔔 Entrega mañana</h2>
        <p>Hola <strong>{$h['periodista_nombre']}</strong>,</p>
        <p>La historia <strong>{$h['titulo']}</strong> debe ser entregada <strong>mañana</strong>.</p>
        <blockquote style='border-left:3px solid #5e6ad2;padding:1rem;margin:1rem 0;background:#111214'>
            <strong>{$h['titulo']}</strong><br>
            Fecha de entrega: " . date('d/m/Y', strtotime($h['fecha_entrega'])) . "
        </blockquote>
        <p>Si ya tienes el borrador listo, ingresa a la plataforma y súbelo.</p>
        <p><a href='" . BASE_URL . "/periodista/entregar.php?id={$h['id']}' style='display:inline-block;padding:10px 20px;background:#5e6ad2;color:#fff;text-decoration:none;border-radius:8px'>Entregar ahora</a></p>
...
// También notificar al admin sobre vencimientos de mañana
...
if ($admin && !empty($vencimientos)) {
    ...
    foreach ($vencimientos as $v) {
        $lista .= "<li>{$v['titulo']} — {$v['nombre']}</li>";
    }
    $subject = "📋 Vencimientos mañana — " . count($vencimientos) . " historias";
